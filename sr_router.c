...
/* handle icmp message */
int sr_handle_icmp_pkt(struct sr_instance* sr,
        uint8_t * packet,
        unsigned int len,
        char* interface)
{
  ...
  else if ((len > (sizeof(sr_ethernet_hdr) + sizeof(sr_ip_hdr))) &&
    ((ip_hdr->ip_p == 0x0006) || (ip_hdr-> 0x0011))) {
        ...
        char* interface) 
{
  ...
  /* match */
    ...
    /* arp miss */
      ...
      arp_req = sr_arpcache_queuereq(&(sr->cache), gateway, packet, len, o_interface);
